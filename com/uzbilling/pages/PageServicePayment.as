...
//			tscroll.y = top.height + 30;
			...
			top.setTitle(ls.getData("mn.tf2"));
...
//				dtHolder.alpha = 0;
		...
		override protected function setData(dx:int):void {
			...
			for(var i:int=1;i<usrDataArr.length;++i) {
				...
				if(arr.length==5) {
					var infoCell:InfoCellServicePayments = new InfoCellServicePayments();
		...
		private function rearrangeCell(e:InfoCellEvent=null):void {
			...
			for(var i:int=0;i<cellHolder.numChildren;++i) {
				var infoCell:InfoCellServicePayments = cellHolder.getChildAt(i) as InfoCellServicePayments;
				if(infoCell) {
					infoCell.y = h;
					h += infoCell.height;
				}
